...
@pytest.fixture
def decoder():
    """Fixture qui crée une instance de ConstrainedDecoder sans charger le LLM réel."""
    mock_llm = MagicMock()
    
    # On patche le __init__ pour éviter les erreurs de "Repo ID" (chargement tokenizer/model)
    with patch.object(ConstrainedDecoder, '__init__', return_value=None):
        obj = ConstrainedDecoder(mock_llm)
        
        # On injecte manuellement les attributs nécessaires au fonctionnement des méthodes
        obj.llm = mock_llm
        obj.quote_id = 99
...
def test_extract_param_value_number_formatting(decoder):
    ...
    # Simulation de la séquence de tokens : '1', '0', puis un token STOP (espace)
    ...
    assert decoder.llm.decode.call_count == 2 # '1' et '0' (le stop break avant le decode final)
...
def test_extract_param_value_string_newline_break(decoder):
    ...
    decoder.llm.get_logits_from_input_ids = MagicMock(return_value=np.zeros(100))
...
def test_string_first_token_masks_quote(decoder):
    ...
    # Il est CRITIQUE que ce soit un array NumPy pour que la modif [id] = NEG_INF fonctionne
    ...
    with patch("numpy.argmax", side_effect=side_effects) as mock_argmax:
        # On mock aussi le decode pour éviter les messages MagicMock dans la console
