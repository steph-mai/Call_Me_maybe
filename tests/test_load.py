...
def test_load_invalid_json_content(tmp_path):
    ...
    d.write_text('{ "prompt": "test" ') # Accolade manquante
...
@pytest.mark.skipif(sys.platform == "win32", reason="chmod non supporté nativement comme sur Unix")
def test_load_permission_denied(tmp_path):
    ...
    d.chmod(0o000) # Supprime tous les droits d'accès
    
    loader = Loader()
    with pytest.raises(PermissionError):
        loader.load_file(str(d))
    
    # On remet les droits pour que tmp_path puisse nettoyer le dossier
    d.chmod(0o644)